...
namespace Keccak
  ...
  def os (c: ValidCircuit P P_Prime) (round: ℕ) (x y: Fin 5) :=
    s c round x y + (Decode.expr (bc c round (x + 4)) + Decode.expr ((bc c round (x+1)).rotateRight (get_rotate_count 1 part_size_c)))

  @[to_os] lemma os_0_0: cell_manager c round 0 +
    (Decode.expr
      ...
      Decode.expr
        [(1, cell_manager c round 259), (3, cell_manager c round 238), (3, cell_manager c round 239),
          ...
          (3, cell_manager c round 258)]) = os c round 0 0
  := by
    have h: (↑(4: Fin 5)) = (4: ℕ) := rfl
    simp [os, bc, c_parts, Transform.split_expr, Split.expr, part_size_c, get_num_bits_per_theta_c_lookup_val, word_parts_known, List.enum, h, get_rotate_count, List.rotateRight, s]

  @[to_os] lemma os_1_0: cell_manager c round 5 +
    (Decode.expr
      ...
      Decode.expr
        [(1, cell_manager c round 281), (3, cell_manager c round 260), (3, cell_manager c round 261),
          ...
          (3, cell_manager c round 280)]) = os c round 1 0
  := by
    have h: (↑(4: Fin 5)) = (4: ℕ) := rfl
    simp [os, bc, c_parts, Transform.split_expr, Split.expr, part_size_c, get_num_bits_per_theta_c_lookup_val, word_parts_known, List.enum, h, get_rotate_count, List.rotateRight, s]
