...
namespace Keccak
  ...
  namespace Gates
    ...
    namespace AbsorbVerifyInput
      lemma gate_82_absorb_verify_input (c: ValidCircuit P P_Prime) (h_fixed: c.1.Fixed = fixed_func c) (hgate: gate_82 c) (h_n: 300 < c.n):
        ∀ round, (round = 0 ∨ round = 25) →
          continue_hash c (12*round) → (
            absorb_from c (round+3) = s c round 2 0
          )
      := by
        unfold gate_82 at hgate
