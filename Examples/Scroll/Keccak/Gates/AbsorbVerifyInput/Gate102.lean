...
namespace Keccak
  ...
  namespace Gates
    ...
    namespace AbsorbVerifyInput
      lemma gate_102_absorb_verify_input (c: ValidCircuit P P_Prime) (h_fixed: c.1.Fixed = fixed_func c) (hgate: gate_102 c) (h_n: 300 < c.n):
        ∀ round, (round = 0 ∨ round = 25) →
          continue_hash c (12*round) → (
            absorb_from c (round+13) = s c round 2 2
          )
      := by
        unfold gate_102 at hgate
        ...
        cases hgate with
          ...
          | inr heq =>
            ...
            rewrite [heq]
            cases hround with
              | inl hround => simp only [hround, Nat.reduceMul, Nat.zero_mod]
