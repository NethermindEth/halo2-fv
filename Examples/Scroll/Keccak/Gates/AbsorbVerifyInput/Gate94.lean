...
namespace Keccak
  ...
  namespace Gates
    ...
    namespace AbsorbVerifyInput
      lemma gate_94_absorb_verify_input (c: ValidCircuit P P_Prime) (h_fixed: c.1.Fixed = fixed_func c) (hgate: gate_94 c):
        ∀ round, (round = 0 ∨ round = 25) →
          continue_hash c (12*round) → (
            absorb_from c (round+9) = s c round 3 1
          )
      := by
        unfold gate_94 at hgate
