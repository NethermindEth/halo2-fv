...
namespace Keccak
  ...
  namespace Gates
    ...
    namespace AbsorbVerifyInput
      lemma gate_96_absorb_verify_input (c: ValidCircuit P P_Prime) (h_fixed: c.1.Fixed = fixed_func c) (hgate: gate_96 c):
        ∀ round, (round = 0 ∨ round = 25) →
          continue_hash c (12*round) → (
            absorb_from c (round+10) = s c round 4 1
          )
      := by
        unfold gate_96 at hgate
