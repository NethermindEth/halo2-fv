...
namespace Keccak
  namespace Gates
    namespace AbsorbStateCopy
      lemma gate_117_absorb_state_copy (c: ValidCircuit P P_Prime) (h_fixed: c.1.Fixed = fixed_func c) (hgate: gate_117 c) (h_is_final_bool: boolean_is_final c) (h_is_final_first: is_final_disabled_on_first_row c) (h_n: 323 < c.n):
        ∀ round, (round = 0 ∨ round = 25) →
          (continue_hash c (12*round) ∧ s c round 2 4 = s c (round+1) 2 4) ∨
          (¬continue_hash c (12*round) ∧ s c (round+1) 2 4 = 0)
        := by
          unfold gate_117 at hgate
          intro round hround
