    ...
    namespace SplitUniform
      ...
      lemma gate_42_43_split_uniform (c: ValidCircuit P P_Prime) (h_fixed: c.1.Fixed = fixed_func c) (hrot_gate: gate_42 c) (hsplit_gate: gate_43 c) (h_n: 299 < c.n):
        ...
        SplitUniform.constraint c (round+1)
          (output_cells := rho_pi_chi_cells c (round+1) 0 4 2)
          (cell_offset := 1624)
          (rot := RHO_MATRIX 0 4)
          (target_part_size := get_num_bits_per_base_chi_lookup)
          (input := os c (round+1) 0 4)
        := by
          ...
          . rewrite [←hsplit_gate]
            ...
            aesop
          . simp [keccak_constants, word_parts_known, get_rotate_count, target_part_sizes_known, SplitUniform.rot_parts, rho_pi_chi_cells, zmod_pow_simps, fin_vals, List.range, List.range.loop, List.rotateRight]
            rewrite [←hrot_gate]
            simp [mul_comm]

      lemma gate_44_aux (c: ValidCircuit P P_Prime) (hrot_gate: gate_44 c) (h_fixed: c.1.Fixed = fixed_func c) (h_n: 299 < c.n) :
        ∀ round ≤ 23,
        cell_manager c (round+1) 1626 + 64 * cell_manager c (round+1) 1627 = cell_manager c (round+1) 688
      := by
      ...
      lemma gate_45_aux (c: ValidCircuit P P_Prime) (hsplit_gate: gate_45 c) (h_fixed: c.1.Fixed = fixed_func c) (h_n: 299 < c.n) :
        ∀ round ≤ 23,
        Decode.expr
          [(2, cell_manager c (round+1) 1627), (4, cell_manager c (round+1) 689), (4, cell_manager c (round+1) 690),
            (4, cell_manager c (round+1) 691), (4, cell_manager c (round+1) 692), (4, cell_manager c (round+1) 693),
            (4, cell_manager c (round+1) 694), (4, cell_manager c (round+1) 695), (4, cell_manager c (round+1) 744),
            (4, cell_manager c (round+1) 745), (4, cell_manager c (round+1) 746), (4, cell_manager c (round+1) 747),
            (4, cell_manager c (round+1) 748), (4, cell_manager c (round+1) 749), (4, cell_manager c (round+1) 750),
            (4, cell_manager c (round+1) 751), (2, cell_manager c (round+1) 1626)] =
        os c (round+1) 1 4
      := by
      ...
      lemma gate_44_45_split_uniform (c: ValidCircuit P P_Prime) (h_fixed: c.1.Fixed = fixed_func c) (hrot_gate: gate_44 c) (hsplit_gate: gate_45 c) (h_n: 299 < c.n):
        ∀ round ≤ 23,
        SplitUniform.constraint c (round+1)
          (output_cells := rho_pi_chi_cells c (round+1) 0 4 4)
          (cell_offset := 1626)
          (rot := RHO_MATRIX 1 4)
          (target_part_size := get_num_bits_per_base_chi_lookup)
          (input := os c (round+1) 1 4)
        := by
          intro round h_round_range
      ...
      lemma gate_46_47_split_uniform (c: ValidCircuit P P_Prime) (h_fixed: c.1.Fixed = fixed_func c) (hrot_gate: gate_46 c) (hsplit_gate: gate_47 c) (h_n: 299 < c.n):
        ∀ round ≤ 23,
        SplitUniform.constraint c (round+1)
          (output_cells := rho_pi_chi_cells c (round+1) 0 4 1)
          (cell_offset := 1628)
          (rot := RHO_MATRIX 2 4)
          (target_part_size := get_num_bits_per_base_chi_lookup)
          (input := os c (round+1) 2 4)
        := by
          ...
          . rewrite [←hsplit_gate]
            ...
            simp [List.rotateRight]
            aesop
          . simp [keccak_constants, word_parts_known, get_rotate_count, target_part_sizes_known, SplitUniform.rot_parts, rho_pi_chi_cells, zmod_pow_simps, List.range, List.range.loop, fin_vals, List.rotateRight]
            rewrite [←hrot_gate]
            simp [mul_comm]
      ...
      lemma gate_48_split_uniform (c: ValidCircuit P P_Prime) (h_fixed: c.1.Fixed = fixed_func c) (hsplit_gate: gate_48 c) (h_n: 299 < c.n):
        ∀ round ≤ 23,
        SplitUniform.constraint c (round+1)
          (output_cells := rho_pi_chi_cells c (round+1) 0 4 3)
          (cell_offset := 1630)
          (rot := RHO_MATRIX 3 4)
          (target_part_size := get_num_bits_per_base_chi_lookup)
          (input := os c (round+1) 3 4)
        := by
          ...
          . rewrite [←hsplit_gate]
            ...
            aesop
          . simp [keccak_constants, word_parts_known, get_rotate_count, target_part_sizes_known, SplitUniform.rot_parts, List.range, List.range.loop, List.rotateRight]
      ...
      lemma gate_49_50_split_uniform (c: ValidCircuit P P_Prime) (h_fixed: c.1.Fixed = fixed_func c) (hrot_gate: gate_49 c) (hsplit_gate: gate_50 c) (h_n: 299 < c.n):
        ∀ round ≤ 23,
        SplitUniform.constraint c (round+1)
          (output_cells := rho_pi_chi_cells c (round+1) 0 4 0)
          (cell_offset := 1630)
          (rot := RHO_MATRIX 4 4)
          (target_part_size := get_num_bits_per_base_chi_lookup)
          (input := os c (round+1) 4 4)
        := by
