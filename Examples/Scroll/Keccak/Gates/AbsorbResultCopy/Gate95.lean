...
namespace Keccak
  ...
  namespace Gates
    ...
    namespace AbsorbVerifyInput
      lemma gate_95_absorb_result_copy (c: ValidCircuit P P_Prime) (h_fixed: c.1.Fixed = fixed_func c) (hgate: gate_95 c) (h_is_final_bool: boolean_is_final c) (h_is_final_first: is_final_disabled_on_first_row c) (h_n: 419 < c.n):
        ∀ round, (round = 0 ∨ round = 25) →
          ((continue_hash c (12*round) ∧ absorb_result c (round+9) = s c (round+1) 3 1) ∨
          ((¬continue_hash c (12*round)) ∧ absorb_data c (round+9) = s c (round+1) 3 1))
      := by
        unfold gate_95 at hgate
        intro round hround
        ...
        have h_factor: 12*round+16 = 12*(round+1)+4 := by trivial
        have h_factor': 12*round+110 = 12*(round+9)+2 := by trivial
        have h_factor'': 12*round+111 = 12*(round+9)+3 := by trivial
        have h_n': 12*(round+1)+11 < c.n := by
          cases hround with
            | inl hround => simp [hround]; linarith
            | inr hround => simp [hround]; linarith
        have h_n'': 12*(round+9)+11 < c.n := by
          cases hround with
            | inl hround => simp [hround]; linarith
