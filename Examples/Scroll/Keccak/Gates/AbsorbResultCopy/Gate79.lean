...
namespace Keccak
  ...
  namespace Gates
    ...
    namespace AbsorbResultCopy
      lemma gate_79_absorb_result_copy (c: ValidCircuit P P_Prime) (h_fixed: c.1.Fixed = fixed_func c) (hgate: gate_79 c) (h_is_final_bool: boolean_is_final c) (h_is_final_first: is_final_disabled_on_first_row c) (h_n: 323 < c.n):
        ∀ round, (round = 0 ∨ round = 25) →
          ((continue_hash c (12*round) ∧ absorb_result c (round+1) = s c (round+1) 0 0) ∨
          ((¬continue_hash c (12*round)) ∧ absorb_data c (round+1) = s c (round+1) 0 0))
      := by
        unfold gate_79 at hgate
        intro round hround
        ...
        have h_factor: 12*round+12 = 12*(round+1) := by trivial
        have h_factor': 12*round+14 = 12*(round+1)+2 := by trivial
        have h_factor'': 12*round+15 = 12*(round+1)+3 := by trivial
        have h_n': 12*(round+1)+11 < c.n := by
          cases hround with
            | inl hround => simp [hround]; omega
            | inr hround => simp [hround]; assumption
        simp [h_factor, h_factor', h_factor'', to_cell_manager, h_n', to_s, to_absorb, to_named_advice] at hgate
