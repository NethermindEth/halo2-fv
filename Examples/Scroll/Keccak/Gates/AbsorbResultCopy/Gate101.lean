...
namespace Keccak
  ...
  namespace Gates
    ...
    namespace AbsorbResultCopy
      lemma gate_101_absorb_result_copy (c: ValidCircuit P P_Prime) (h_fixed: c.1.Fixed = fixed_func c) (hgate: gate_101 c) (h_is_final_bool: boolean_is_final c) (h_is_final_first: is_final_disabled_on_first_row c) (h_n: 455 < c.n):
        ∀ round, (round = 0 ∨ round = 25) →
          ((continue_hash c (12*round) ∧ absorb_result c (round+12) = s c (round+1) 1 2) ∨
          ((¬continue_hash c (12*round)) ∧ absorb_data c (round+12) = s c (round+1) 1 2))
      := by
        unfold gate_101 at hgate
        intro round hround
        ...
        have h_factor': 12*round+146 = 12*(round+12)+2 := by trivial
        have h_factor'': 12*round+147 = 12*(round+12)+3 := by trivial
        have h_n': 12*(round+1)+11 < c.n := by
          cases hround with
            | inl hround => simp [hround]; omega
            | inr hround => simp [hround]; omega
        have h_n'': 12*(round+12)+11 < c.n := by
          cases hround with
            | inl hround => simp [hround]; omega
