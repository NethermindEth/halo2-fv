...
namespace Keccak
  ...
  namespace Gates
    ...
    namespace AbsorbResultCopy
      lemma gate_107_absorb_result_copy (c: ValidCircuit P P_Prime) (h_fixed: c.1.Fixed = fixed_func c) (hgate: gate_107 c) (h_is_final_bool: boolean_is_final c) (h_is_final_first: is_final_disabled_on_first_row c) (h_n: 491 < c.n):
        ∀ round, (round = 0 ∨ round = 25) →
          ((continue_hash c (12*round) ∧ absorb_result c (round+15) = s c (round+1) 4 2) ∨
          ((¬continue_hash c (12*round)) ∧ absorb_data c (round+15) = s c (round+1) 4 2))
      := by
        unfold gate_107 at hgate
        intro round hround
        ...
        have h_factor: 12*round+22 = 12*(round+1)+10 := by trivial
        have h_factor': 12*round+182 = 12*(round+15)+2 := by trivial
        have h_factor'': 12*round+183 = 12*(round+15)+3 := by trivial
        have h_n': 12*(round+1)+11 < c.n := by
          cases hround with
            | inl hround => simp [hround]; linarith
            | inr hround => simp [hround]; linarith
        have h_n'': 12*(round+15)+11 < c.n := by
          cases hround with
            | inl hround => simp [hround]; linarith
