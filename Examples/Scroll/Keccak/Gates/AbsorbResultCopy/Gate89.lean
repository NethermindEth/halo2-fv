...
namespace Keccak
  ...
  namespace Gates
    ...
    namespace AbsorbVerifyInput
      lemma gate_89_absorb_result_copy (c: ValidCircuit P P_Prime) (h_fixed: c.1.Fixed = fixed_func c) (hgate: gate_89 c) (h_is_final_bool: boolean_is_final c) (h_is_final_first: is_final_disabled_on_first_row c) (h_n: 383 < c.n):
        ∀ round, (round = 0 ∨ round = 25) →
          ((continue_hash c (12*round) ∧ absorb_result c (round+6) = s c (round+1) 0 1) ∨
          ((¬continue_hash c (12*round)) ∧ absorb_data c (round+6) = s c (round+1) 0 1))
      := by
        unfold gate_89 at hgate
        intro round hround
        ...
        have h_factor': 12*round+74 = 12*(round+6)+2 := by trivial
        have h_factor'': 12*round+75 = 12*(round+6)+3 := by trivial
        have h_n': 12*(round+1)+11 < c.n := by
          cases hround with
            | inl hround => simp [hround]; linarith
            | inr hround => simp [hround]; linarith
        have h_n'': 12*(round+6)+11 < c.n := by
          cases hround with
            | inl hround => simp [hround]; linarith
