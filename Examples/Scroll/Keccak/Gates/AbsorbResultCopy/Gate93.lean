...
namespace Keccak
  ...
  namespace Gates
    ...
    namespace AbsorbVerifyInput
      lemma gate_93_absorb_result_copy (c: ValidCircuit P P_Prime) (h_fixed: c.1.Fixed = fixed_func c) (hgate: gate_93 c) (h_is_final_bool: boolean_is_final c) (h_is_final_first: is_final_disabled_on_first_row c) (h_n: 407 < c.n):
        ∀ round, (round = 0 ∨ round = 25) →
          ((continue_hash c (12*round) ∧ absorb_result c (round+8) = s c (round+1) 2 1) ∨
          ((¬continue_hash c (12*round)) ∧ absorb_data c (round+8) = s c (round+1) 2 1))
      := by
        unfold gate_93 at hgate
        intro round hround
        ...
        have h_factor: 12*round+23 = 12*(round+1)+11 := by trivial
        have h_factor': 12*round+98 = 12*(round+8)+2 := by trivial
        have h_factor'': 12*round+99 = 12*(round+8)+3 := by trivial
        have h_n': 12*(round+1)+11 < c.n := by
          cases hround with
            | inl hround => simp [hround]; linarith
            | inr hround => simp [hround]; linarith
        have h_n'': 12*(round+8)+11 < c.n := by
          cases hround with
            | inl hround => simp [hround]; linarith
