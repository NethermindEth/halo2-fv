    ...
    namespace AbsorbResultCopy
      lemma gate_83_absorb_result_copy (c: ValidCircuit P P_Prime) (h_fixed: c.1.Fixed = fixed_func c) (hgate: gate_83 c) (h_is_final_bool: boolean_is_final c) (h_is_final_first: is_final_disabled_on_first_row c) (h_n: 347 < c.n):
        ∀ round, (round = 0 ∨ round = 25) →
          ...
          ((¬continue_hash c (12*round)) ∧ absorb_data c (round+3) = s c (round+1) 2 0))
      := by
        unfold gate_83 at hgate
        intro round hround
        ...
        have h_factor': 12*round+38 = 12*(round+3)+2 := by trivial
        have h_factor'': 12*round+39 = 12*(round+3)+3 := by trivial
        have h_n': 12*(round+1)+11 < c.n := by
          cases hround with
            | inl hround => simp [hround]; omega
            | inr hround => simp [hround]; omega
        have h_n'': 12*(round+3)+11 < c.n := by
          cases hround with
            | inl hround => simp [hround]; omega
