...
namespace Keccak
  ...
  namespace Gates
    ...
    namespace AbsorbResultCopy
      lemma gate_97_absorb_result_copy (c: ValidCircuit P P_Prime) (h_fixed: c.1.Fixed = fixed_func c) (hgate: gate_97 c) (h_is_final_bool: boolean_is_final c) (h_is_final_first: is_final_disabled_on_first_row c) (h_n: 431 < c.n):
        ∀ round, (round = 0 ∨ round = 25) →
          ((continue_hash c (12*round) ∧ absorb_result c (round+10) = s c (round+1) 4 1) ∨
          ((¬continue_hash c (12*round)) ∧ absorb_data c (round+10) = s c (round+1) 4 1))
      := by
        unfold gate_97 at hgate
        intro round hround
        ...
        have h_factor: 12*round+21 = 12*(round+1)+9 := by trivial
        have h_factor': 12*round+122 = 12*(round+10)+2 := by trivial
        have h_factor'': 12*round+123 = 12*(round+10)+3 := by trivial
        have h_n': 12*(round+1)+11 < c.n := by
          cases hround with
            | inl hround => simp [hround]; omega
            | inr hround => simp [hround]; omega
        have h_n'': 12*(round+10)+11 < c.n := by
          cases hround with
            | inl hround => simp [hround]; omega
