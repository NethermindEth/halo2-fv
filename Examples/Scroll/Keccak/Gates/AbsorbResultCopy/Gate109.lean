...
namespace Keccak
  ...
  namespace Gates
    ...
    namespace AbsorbVerifyInput
      lemma gate_109_absorb_result_copy (c: ValidCircuit P P_Prime) (h_fixed: c.1.Fixed = fixed_func c) (hgate: gate_109 c) (h_is_final_bool: boolean_is_final c) (h_is_final_first: is_final_disabled_on_first_row c) (h_n: 503 < c.n):
        ∀ round, (round = 0 ∨ round = 25) →
          ((continue_hash c (12*round) ∧ absorb_result c (round+16) = s c (round+1) 0 3) ∨
          ((¬continue_hash c (12*round)) ∧ absorb_data c (round+16) = s c (round+1) 0 3))
      := by
        unfold gate_109 at hgate
        intro round hround
        ...
        have h_factor': 12*round+194 = 12*(round+16)+2 := by trivial
        have h_factor'': 12*round+195 = 12*(round+16)+3 := by trivial
        have h_n': 12*(round+1)+11 < c.n := by
          cases hround with
            | inl hround => simp [hround]; linarith
            | inr hround => simp [hround]; linarith
        have h_n'': 12*(round+16)+11 < c.n := by
          cases hround with
            | inl hround => simp [hround]; linarith
