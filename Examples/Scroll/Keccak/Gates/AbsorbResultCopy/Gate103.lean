...
namespace Keccak
  ...
  namespace Gates
    ...
    namespace AbsorbVerifyInput
      lemma gate_103_absorb_result_copy (c: ValidCircuit P P_Prime) (h_fixed: c.1.Fixed = fixed_func c) (hgate: gate_103 c) (h_is_final_bool: boolean_is_final c) (h_is_final_first: is_final_disabled_on_first_row c) (h_n: 467 < c.n):
        ∀ round, (round = 0 ∨ round = 25) →
          ((continue_hash c (12*round) ∧ absorb_result c (round+13) = s c (round+1) 2 2) ∨
          ((¬continue_hash c (12*round)) ∧ absorb_data c (round+13) = s c (round+1) 2 2))
      := by
        unfold gate_103 at hgate
        intro round hround
        ...
        have h_factor': 12*round+158 = 12*(round+13)+2 := by trivial
        have h_factor'': 12*round+159 = 12*(round+13)+3 := by trivial
        have h_n': 12*(round+1)+11 < c.n := by
          cases hround with
            | inl hround => simp [hround]; linarith
            | inr hround => simp [hround]; linarith
        have h_n'': 12*(round+13)+11 < c.n := by
          cases hround with
            | inl hround => simp [hround]; linarith
