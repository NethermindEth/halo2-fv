...
namespace Keccak
  ...
  def intermediate_data_rlc (c: ValidCircuit P P_Prime) (round: Finset.Icc 1 17): Prop :=
    ∀ idx,
      (is_paddings c round idx = 0 ∧ data_rlc c (12*↑round + NUM_BYTES_PER_WORD - (↑idx + 1)) = data_rlc c (12*round + NUM_BYTES_PER_WORD - ↑idx) * c.get_challenge 1 0 + (input_bytes c round).1[idx].2) ∨
      (is_paddings c round idx = 1 ∧ data_rlc c (12*↑round + NUM_BYTES_PER_WORD - (↑idx + 1)) = data_rlc c (12*round + NUM_BYTES_PER_WORD - ↑idx)) -- 8 = NUM_BYTES_PER_WORD
...
-- 0x827364
-- 0x827364   100000001
-- 0b01010010100100101010101
-- 0b01010010100100101010101|100000000000000000000000000000000000000000000000001
-- 0b00000001|10000000000000000000000000000000001
-- 0bXXXXXXXX 00000001|10000000000000000000000001
...
-- 11
  ...
  --ASSUMPTION: ∀ list, list.length ≤ 136 → list ≠ INPUT_LIST → RLC (list) ≠ INPUT_RLC
  -- data_rlc (get_num_rows_per_round*25) = INPUT_RLC
  -- h_input: data_rlc (get_num_rows_per_round*17) = INPUT_RLC by data_rlc_equality_check
  -- if INPUT_RLC = 0
  --   INPUT_BYTES = [] (by ASSUMPTION)
  --   if is_paddings c round idx = 0
  --     data_rlc c (12*round + NUM_BYTES_PER_WORD - idx - 1) = 0 * challenge + input_bytes
  -- if start_new_hash (get_num_rows_per_round * 16)
  --   data_rlc c (get_num_rows_per_round*17+NUM_BYTES_PER_WORD) = 0
  --   INPUT = 0 by h_input
  --   H_1: INPUT ≠ 0
  --   contradiction
