...
namespace Keccak
  lemma input_bytes_lentgh: (input_bytes c round).length = 8 := by
    unfold input_bytes Transform.split_expr Split.expr
    simp [keccak_constants, word_parts_known]

  def get_input_byte_expr (c: ValidCircuit P P_Prime) (round: ℕ): (Fin 8) → ZMod P
    | x => (input_bytes c round)[x].2

  lemma get_input_byte_expr_0: get_input_byte_expr c round 0 = cell_manager c round 72 := by
    simp [get_input_byte_expr, input_bytes, Transform.split_expr, keccak_constants, Split.expr]
  ...
  lemma get_input_byte_expr_3: get_input_byte_expr c round 3 = cell_manager c round 75 := by
    simp [get_input_byte_expr, input_bytes, Transform.split_expr, keccak_constants, Split.expr]; rfl
